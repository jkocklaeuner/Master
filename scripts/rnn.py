...
parser.add_argument('-sub','--subsize',default = 1, type = int, help = "Size of each subnetworks, integer number of spatial orbitals. Should be an integer divisor of the spatial orbitals")
...
parser.add_argument('-mem','--memory',type=int,default = 64, help  = "Number of recurrent memory units for each recurrent layer")
...
parser.add_argument('-l','--layers',type=int,default = 2, help = "Number of Dense layers of the phase network (+ 1 output layer of size 4)")
...
f"Model parameters: \n" 
f"System RNN \n"
f"Number of recurrent layers 3 \n" 
f"Dilation (1,2,3) \n"
f"Memory units per layer {mem_units} \n"
...
ma = RNN(hilbert=hi, layers=n_layers, features=n_features,subsize=subsize,use_phase=True,dtype=jnp.float32)

#Sampler

sa = RNNWeightedSampler(hi)
